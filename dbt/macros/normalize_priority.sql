...
{% macro normalize_priority(col) %}
    case
        when lower(cast({{ col }} as varchar)) in ('1', '2', '3', '4') then cast({{ col }} as integer)
...
{% macro priority_reject_reason(col) %}
    case
        ...
        when lower(cast({{ col }} as varchar)) in ('p1', 'unknown') then 'priority là nhãn không hợp lệ (' || cast({{ col }} as varchar) || ')'
        when try_cast({{ col }} as integer) is not null and try_cast({{ col }} as integer) not between 1 and 4 then 'priority số nằm ngoài khoảng 1..4 (' || cast({{ col }} as varchar) || ')'
